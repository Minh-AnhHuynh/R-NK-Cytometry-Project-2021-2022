...
files = list.files("./02_data/NK_FCS/",pattern="fcs",full.names = TRUE)
...
mst_graph = igraph:::read.graph(paste0(output_dir,"mst.gml",sep=.Platform$file.sep),format="gml")

#plot(mst_GRAPH)
SPADE.plot.trees(mst_graph, output_dir,file_pattern="*fcs*Rsave",layout=as.matrix(LAYOUT_TABLE),out_dir=paste0(output_dir, "/PDF/"),size_scale_factor=1)